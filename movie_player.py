...
def main(movie_fpath:str, play_fps:float, img_proc:ImageProc):

    # 画像出力用フォルダ作成
    output_imgdir_name:str = os.path.splitext(os.path.basename(movie_fpath))[0]
    output_imgdir_path:str = f"./test_data/{img_proc.toString()}/" + output_imgdir_name
    ...
    if os.path.isfile(movie_fpath) == False:
        ...
        num_frame:int = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        cap_fps:float = cap.get(cv2.CAP_PROP_FPS)

        frame_play_step:int = int((cap_fps + 0.1) / play_fps)
        ...
        while frame_no < num_frame:
            ...
            if frame_no % frame_play_step == 0:
                ...
                if not (frame_img is None):
                    ...
                    # 保存
                    frame_img_fpath = f"{output_imgdir_path}/{output_imgdir_name}_{img_proc.toString()}_F{frame_no:05}.jpg" 
...
if __name__ == "__main__":
    ...
    # img_proc = ImageProc(500, 250, 1100, 500)
